...
# Define Vagrant configuration for Vagrantfile
cat <<EOF > Vagrantfile
       ...
       #Install packages on the Master VM
        master.vm.provision "shell", inline: <<-SHELL
            ...
            if ! id -u ALTSCHOOL &> /dev/null; then
                sudo useradd -m -G sudo -p $ALTSCHOOL_PASSWORD ALTSCHOOL
            fi
            ...
            if sudo -u ALTSCHOOL sshpass -p "$SSH_PASSWORD" ssh -o StrictHostKeyChecking=no vagrant@$SLAVE_IP "mkdir -p /home/Altschool_dir/.ssh && echo $(sudo -u ALTSCHOOL cat /home/Altschool_dir/.ssh/id_rsa.pub) >> /home/Altschool_dir/.ssh/authorized_keys"; then
            ...
            if sudo -u ALTSCHOOL sshpass -p "$SSH_PASSWORD" ssh-copy-id -i /home/Altschool_dir/.ssh/id_rsa.pub vagrant@$SLAVE_IP; then
                echo "Successfully copied the public key to the slave VM"
            else
                echo "Failed to copy the public key to the slave VM"
                exit 1
           ...
           if ! sudo cp /home/Altschool_dir/.ssh/id_rsa.pub /mnt/altschool/master; then
            ...
            if sudo -u ALTSCHOOL sshpass -p "$SSH_PASSWORD" ssh vagrant@$SLAVE_IP mount | grep -q /mnt/altschool/master; then
